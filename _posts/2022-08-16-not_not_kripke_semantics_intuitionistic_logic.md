...
date: 2023-08-16
permalink: /posts/2022/08/kripke-intuitionism/
tags:
  - Kripke semantics
...
In this post we motivate the study of Kripke semantics for
propositional
...
The following axioms were extract from (Hodel 2013):
...
- *Language*: The symbols of the language are:
  ...
  - $$\neg, \rightarrow, \land, \lor$$: negation, implication,
    conjunction, and disjunction symbol
...
- $$B$$ is obtained from $$C$$ and $$C \rightarrow B$$ by an application
...
- Base case: Since the base case accounts to axioms in $$J$$, we need to
  ...
  - Case $$J2$$: To prove
    ...
    - Case 2. $$\phi(A) < \phi(B \rightarrow C)$$. The latter implies
      that
      ...
      - Subcase 2. $$\phi(B) < \phi(C)$$: It is enough to prove that
        ...
        \<`<observation>`{=html}\>observation: For any two formulas
  ...
  - Case $$J8$$: To prove that
    ...
    - Case 1. $$\phi(A) \geq \phi(C)$$: It is enough to prove that
      ...
      i.e. $$\phi(B \rightarrow C) \geq \phi((A \lor B) \rightarrow C)$$.
      ...
      - Subcase 2. $$\phi(B) < \phi(C)$$. Since
        ...
        $$\phi(B \rightarrow C) \geq \phi((A \lor B) \rightarrow C)$$.

    - Case 2. $$\phi(A) < \phi(B \rightarrow C)$$. We will prove the
      following observation \<`<observation2>`{=html}\>observation:
      $$\phi((A \land B) \rightarrow C) = \phi(A \rightarrow (B \rightarrow C))$$.

      - Subcase 1. $$\phi(A) \geq \phi(B \rightarrow C)$$: This means
      ...
      - Subase 2. $$\phi(A) < \phi(B \rightarrow C)$$: This implies that
        $$\phi(A \rightarrow (B \rightarrow C)) = \phi(B \rightarrow C)$$.
...
We notice that with an assignment
...
the previous
theorem can be considered an **invariant** for the
...
Proof:
We use the $$n+1$$-valued logic previously defined in [Theorem
1](#theorem1).
We notice that
$$\phi(D_n) = \min_{1 \leq i<j \leq n} \{ \phi(p_i \leftrightarrow p_j) \}$$.
Let us suppose by contradiction that $$\vdash_J D_n$$. Thus, by [Theorem
1](#theorem1)
...
from [Lemma 1](#lemma1), so there is a truth assignment $$\phi$$
...
Let us prove the following theorem in $$J$$:
$$\vdash_J p_k \leftrightarrow p_k$$.
...
Definition:(Dalen 2013)
A *Kripke model* is a tuple $$\mathcal{K} = \langle K, \Sigma \rangle$$
such that
...
1.  $$\psi \lor \phi \in \Sigma(k)$$ if and only if
    $$\psi \in \Sigma(k)$$
    or $$\phi \in \Sigma(k)$$
2.  $$\psi \land \phi \in \Sigma(k)$$ if and only if
    $$\psi \in \Sigma(k)$$
    and $$\phi \in \Sigma(k)$$
3.  $$\psi \rightarrow \phi \in \Sigma(k)$$ if and only if for all
    $$l \geq k$$
...
$$l \models \phi_1 \rightarrow \phi_2$$.
...
Dalen, Dirk van. 2013. *Logic and Structure*. Springer London.
<https://doi.org/10.1007/978-1-4471-4558-5>.



Hodel, R. E. 2013. *An Introduction to Mathematical Logic*. Dover Books
on Mathematics. Dover Publications, Incorporated.
<https://books.google.com/books?id=SxRYdzWio84C>.



Richardson, George P. 2011. "Reflections on the Foundations of System
Dynamics." *System Dynamics Review* 27 (3): 219--43.
https://doi.org/<https://doi.org/10.1002/sdr.462>.



Trudeau, Richard J. 2001. "The Possibility of Non-Euclidean Geometry."
In *The Non-Euclidean Revolution: With an Introduction by h.s.m
Coxeter*. Birkhäuser Boston.
<https://doi.org/10.1007/978-1-4612-2102-9_5>.
